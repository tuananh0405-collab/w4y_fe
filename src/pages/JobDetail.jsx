...
const JobDetail = () => {
  ...
  const handleApply = async () => {
    ...
    try {
      ...
      if (response.success) alert("Ứng tuyển thành công!");
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="flex flex-col w-full" >
      ...
      <div className="flex p-8" style={{background:theme.colors.bgColor}}>
        {/* Thông tin công việc */}
        <div className="flex flex-col p-8 flex-1 mr-4" style={{ color: colors.veryDarkGreen }}>
          <h1 className="text-4xl font-bold mb-6" style={{ color: colors.tealGreen }}>
            {job?.title}
          </h1>

          <p className="text-lg mb-8">{job?.description}</p>

          <h2 className="text-2xl font-bold mb-4" style={{ color: colors.tealGreen }}>
            Cách thức ứng tuyển
          </h2>
        ...
        >
          <div className="mb-8">
            <h3 className="text-2xl font-bold mb-4" style={{ color: colors.tealGreen }}>
              Thông tin công việc
            </h3>
            <div className="mb-2">
