...
const RecruiterSuggestionList = ({ userId, onSelect }) => {
  const {
    data: jobApplicantsListQuery,
    error: fetchError,
    isLoading: isFetchingList,
  } = useGetApplicantsGroupedByApplicationsQuery({ recruiterId: userId });
    ...
    <div className="grow flex flex-col gap-4 overflow-auto">
      {jobApplicantsList.map((jobApplicantsRecord) => (
        <SuggestionListItem
          ...
          descriptionsStack={jobApplicantsRecord.applications.map((
            application,
            index,
          ) =>
            `${index + 1
            }: ${application.job.title}, trạng thái: ${application.applicationStatus}`
          )}
          onSelect={onSelect}
        />
      ))}
