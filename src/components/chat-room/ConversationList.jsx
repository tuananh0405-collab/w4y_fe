...
import { useGetConversationsQuery } from "../../redux/api/chatApiSlice";

// List of available conversations
const ConversationList = ({ senderId, onSelect, query, isChangingQuery }) => {
  const { data: userListQuery, error: fetchError, isLoading: isFetchingList } =
    useGetConversationsQuery({ senderId, query });
  ...
  return (
    <div className="grow flex flex-col gap-2 overflow-auto">
      {!query && (
        <Typography variant="body1" paddingX={2} className="w-full font-light">
      ...
      <Stack>
        {userList.map((user) => (
          <ConversationListItem
            ...
            onSelect={() => onSelect(user.receiverId)}
          />
        ))}
      </Stack>
    </div>
